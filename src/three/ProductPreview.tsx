...
export function ProductPreview({ product, colorwayIndex, interactive = false }: ProductPreviewProps) {
  return (
    ...
    >
      ...
      <directionalLight position={[-4, 2, -3]} intensity={0.35} color="#d98a3d" />
      <Suspense fallback={null}>
        <group position={[0, -0.1, 0]}>
          <Tumbler product={product} colorwayIndex={colorwayIndex} active spin={!interactive} />
        </group>
        <ContactShadows position={[0, -0.11, 0]} opacity={0.55} scale={4} blur={2.4} far={2} />
        {/* synthetic studio env — no external HDR fetch */}
        <Environment resolution={128}>
          <Lightformer intensity={2.2} color="#f5f1ea" position={[0, 3, 2]} scale={[4, 2, 1]} />
          <Lightformer intensity={1} color="#d98a3d" position={[-3, 1, -2]} scale={[3, 3, 1]} />
        </Environment>
      </Suspense>
      {interactive && (
        <OrbitControls
